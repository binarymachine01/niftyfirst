...
const ALL_CATEGORIES = [
  { name: 'Insider Trading', desc: 'Promoter & Key Executive Disclosures', color: 'text-emerald-400', badge: 'SEBI PIT' },
  { name: 'SAST Deals', desc: 'Substantial Acquisition & Takeovers', color: 'text-purple-400', badge: 'Takeovers' },
  { name: 'Block Deals', desc: 'Institutional Exchange Block Window', color: 'text-amber-400', badge: 'Min ₹10Cr' },
  { name: 'Bulk Deals', desc: 'Large Market Trades (>0.5% Equity)', color: 'text-cyan-400', badge: 'High Volume' },
];
    ...
    <div className="glass-panel p-6 space-y-6">
      ...
      <div className="flex items-center justify-between pb-4 border-b border-white/[0.08]">
        <div className="flex items-center gap-3">
          <div className="p-2.5 rounded-xl bg-cyan-500/10 text-cyan-400 border border-cyan-500/25 shadow-sm shadow-cyan-500/10">
            <Sliders className="w-5 h-5" />
          </div>
          <div>
            <h2 className="text-base font-extrabold text-white tracking-tight">Strategy Builder</h2>
            <p className="text-xs text-slate-400">Configure quantitative parameters & filters</p>
          </div>
        </div>
        ...
        {presets.length > 0 && (
          <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-amber-500/10 border border-amber-500/25 text-amber-300 text-xs font-semibold">
            ...
            <select
              ...
              defaultValue=""
              className="bg-transparent border-none text-amber-200 font-semibold text-xs outline-none cursor-pointer"
            >
              <option value="" disabled className="bg-slate-900 text-slate-300">Strategy Templates...</option>
              {presets.map((p) => (
      ...
      >
              ...
              return (
                <button
                  ...
                  className={`flex items-center justify-between p-3 rounded-xl text-left border transition-all duration-200 ${
                    checked
                      ? 'bg-gradient-to-r from-cyan-500/15 to-blue-500/10 text-white border-cyan-500/40 shadow-md shadow-cyan-500/5'
                      : 'bg-slate-900/50 text-slate-400 border-white/[0.06] hover:border-white/20 hover:text-slate-300'
                  }`}
                >
                  ...
                  <span className={`text-[10px] font-bold px-2 py-0.5 rounded-md border flex-shrink-0 ml-2 ${
                    checked ? 'bg-cyan-500/20 text-cyan-300 border-cyan-500/30' : 'bg-slate-800 text-slate-500 border-white/5'
                  }`}>
                    {cat.badge}
                  </span>
                </button>
        ...
        <div>
          ...
          <div className="grid grid-cols-3 gap-2 p-1 rounded-xl bg-slate-950/70 border border-white/[0.08]">
            {[
                ...
                <button
                  ...
                  className={`py-2 px-3 rounded-lg text-center transition-all ${
                    active
                      ...
                      : 'text-slate-400 hover:text-slate-200 border border-transparent text-xs font-medium'
                  }`}
                >
                  <div className="text-xs">{act.label}</div>
                  <div className="text-[10px] opacity-70 font-mono">{act.desc}</div>
                </button>
              );
            })}
          </div>
        ...
        <div className="p-4 rounded-xl bg-slate-900/60 border border-white/[0.06] space-y-3">
          <div className="flex items-center justify-between">
            <label className="flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider text-slate-300">
              <Clock className="w-4 h-4 text-cyan-400" /> Holding Window
            </label>
            <div className="flex items-center gap-1">
              <span className="text-lg font-black font-mono text-cyan-400">{config.holding_days}</span>
              <span className="text-xs font-medium text-slate-400">Trading Days</span>
            </div>
          </div>
        ...
        <div>
          <button
            ...
            className="flex items-center justify-between w-full py-2 px-3 rounded-lg bg-slate-900/40 hover:bg-slate-900 text-xs font-semibold text-slate-400 hover:text-slate-200 border border-white/5 transition-colors"
          >
          ...
          {showAdvanced && (
            <div className="mt-3 p-4 rounded-xl bg-slate-950/80 border border-white/[0.08] space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider text-slate-300 mb-1.5">
                    <Wallet className="w-3.5 h-3.5 text-blue-400" /> Min Value (₹ Lakhs)
                  </label>
                  <input
                    type="number"
                    min="0"
        ...
        <button
          type="submit"
          disabled={loading}
          className="btn-primary w-full py-3.5 text-sm uppercase tracking-wider disabled:opacity-50"
        >
          {loading ? (
