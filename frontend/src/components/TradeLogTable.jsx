...
export default function TradeLogTable({ trades }) {
  ...
  return (
    <div className="glass-panel p-6 mt-6 rounded-2xl">
      ...
      <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4 pb-4 border-b border-white/[0.08] mb-5">
        <div>
          <div className="flex items-center gap-2.5">
            <h3 className="text-base font-extrabold text-white tracking-tight">
              Trade Execution Log
            </h3>
            <span className="text-xs font-mono font-bold px-2.5 py-0.5 rounded-full bg-cyan-500/10 text-cyan-300 border border-cyan-500/25">
              {filteredTrades.length} of {trades.length} Deals
            </span>
          </div>
          <p className="text-xs text-slate-400 mt-0.5">Chronological trade performance breakdown</p>
        </div>

        <div className="flex flex-wrap items-center gap-2.5 w-full lg:w-auto">
          {/* Quick Filter Tabs */}
          <div className="flex items-center p-1 rounded-xl bg-slate-950/70 border border-white/[0.08] text-xs">
            {[
              { id: 'ALL', label: 'All' },
              { id: 'WIN', label: 'Wins' },
      ...
      {/* Table */}
      <div className="overflow-x-auto max-h-[420px] rounded-xl border border-white/[0.06]">
        <table className="w-full text-left text-xs border-collapse">
          <thead className="sticky top-0 z-10 bg-[#0c1222] text-slate-400 uppercase text-[10px] tracking-wider border-b border-white/[0.08]">
            <tr>
              ...
              return (
                <tr key={idx} className="hover:bg-white/[0.025] transition-colors">
                  <td className="py-3 px-3.5">
                    <div className="font-bold text-white text-xs tracking-tight">{t.symbol}</div>
                  ...
                  <td className="py-3 px-3.5 font-sans text-slate-300 truncate max-w-[150px]" title={t.client_name}>
                    {t.client_name || 'N/A'}
                  </td>
                  <td className="py-3 px-3.5 font-sans">
                  ...
                  <td className="py-3 px-3.5">
                    <div className="text-slate-200">₹{t.entry_price.toFixed(2)}</div>
                    <div className="text-[10px] text-slate-500">{t.entry_date}</div>
                  </td>
                  <td className="py-3 px-3.5">
                    <div className="text-slate-200">₹{t.exit_price.toFixed(2)}</div>
                    <div className="text-[10px] text-slate-500">{t.exit_date}</div>
                  </td>
                  <td className="py-3 px-3.5 text-center text-slate-300 font-bold">
                    {t.holding_days}d
                  </td>
                  ...
                  <td className="py-3 px-3.5 font-sans text-slate-400 text-[11px]">
