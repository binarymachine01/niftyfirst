...
export default function StockInspector() {
  ...
  const fetchStockData = async (sym) => {
    ...
    try {
      ...
      console.error(err);
      setError(`Could not load price candles for symbol '${sym}'. Please ensure it is present in the NSE Bhavcopy database.`);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="glass-panel p-6 rounded-2xl">
        ...
        {/* Quick Stock Tags */}
        <div className="flex flex-wrap items-center gap-2 mt-4 pt-3 border-t border-white/[0.06] text-xs text-slate-400">
          <span className="font-semibold text-slate-300">Quick Select:</span>
      ...
      {error && (
        <div className="p-4 rounded-xl bg-rose-500/10 border border-rose-500/30 text-rose-400 text-xs flex items-center gap-2 font-medium">
      ...
      {history.length > 0 && (
        <div className="glass-panel p-6 rounded-2xl">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mb-5 pb-4 border-b border-white/[0.06]">
            <div>
              <div className="flex items-center gap-3">
                <h3 className="text-2xl font-black font-mono text-white tracking-tight">{symbol}</h3>
                {latestCandle && (
                ...
                <span className={`text-xs font-mono font-bold px-2 py-0.5 rounded-md border ${
                  overallReturn >= 0 ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : 'bg-rose-500/10 text-rose-400 border-rose-500/20'
                }`}>
            ...
            <div className="flex items-center gap-3 text-xs font-mono">
              <div className="px-3 py-1.5 rounded-xl bg-slate-900/90 border border-white/[0.08]">
                <span className="text-slate-400">Low: </span>
                <span className="font-bold text-rose-400">₹{minClose.toFixed(2)}</span>
                <span className="text-slate-600 mx-2">|</span>
                <span className="text-slate-400">High: </span>
                <span className="font-bold text-emerald-400">₹{maxClose.toFixed(2)}</span>
              </div>
            </div>
          </div>
                ...
                <Tooltip
                  content={({ active, payload, label }) => {
                    if (active && payload && payload.length) {
                      ...
                      return (
                        <div className="bg-[#0c1222]/95 border border-sky-500/40 p-3.5 rounded-xl shadow-2xl backdrop-blur-xl text-xs font-mono">
                          <div className="text-slate-400 mb-1.5 font-sans font-medium">{label}</div>
                          <div className="grid grid-cols-2 gap-x-5 gap-y-1 text-xs">
                            ...
                            <div>High: <span className="text-emerald-400 font-bold">₹{d.high.toFixed(2)}</span></div>
                            <div>Low: <span className="text-rose-400 font-bold">₹{d.low.toFixed(2)}</span></div>
                            <div>Close: <span className="text-cyan-300 font-bold">₹{d.close.toFixed(2)}</span></div>
                          </div>
                          <div className="text-[10px] text-slate-400 mt-2 pt-1.5 border-t border-white/10 flex justify-between">
      ...
      <div className="glass-panel p-6 rounded-2xl">
        ...
        {deals.length === 0 ? (
          <p className="text-xs text-slate-500 py-6 text-center">
            No insider deals or block trades currently recorded for this stock in database.
          </p>
        ) : (
          <div className="overflow-x-auto rounded-xl border border-white/[0.06]">
            <table className="w-full text-left text-xs border-collapse font-mono">
              <thead className="bg-[#0c1222] text-slate-400 uppercase text-[10px] tracking-wider">
                <tr>
              ...
              <tbody className="divide-y divide-white/[0.04]">
                {deals.map((d) => (
                  <tr key={d.id} className="hover:bg-white/[0.02]">
                    <td className="py-2.5 px-3.5 text-slate-300">{d.trade_date}</td>
                    <td className="py-2.5 px-3.5 font-sans">
                      <span className="badge-tag">{d.deal_category}</span>
                    </td>
                    <td className="py-2.5 px-3.5 font-sans text-slate-300">{d.client_name}</td>
                    <td className="py-2.5 px-3.5 font-sans">
                    ...
                    <td className="py-2.5 px-3.5 text-right text-cyan-300 font-bold">
